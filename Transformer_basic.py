...
class PositionalEncoding(tf.keras.layers.Layer):
  ...
  def get_angles(self, position, i, d_model): # 각도값 계산
...
# 패딩 마스크 구현 : 입력된 정수 시퀀스에서 패딩 토큰의 인덱스인지 판별
# 정수 시퀀스에서 0인 경우 1로 변환, 그렇지 않으면 0 변환.
...
# 포지셔널 인코딩 후 디코더 층 num_layers 만큼 쌓음
...
#트랜스포머 구현
#인코더 출력은 디코더로 전달하여 인코더-디코더 어텐션에 사용
#디코더 끝단에 다중 클래스 분류 문제를 풀기 위해 vocab_size 만큼 뉴런 출력층 추가
...
# 학습률 스케쥴러 방법
# 사용자 정의 단계까지 학습률 증가 / 단계 이후엔 학습률을 점차 감소시킴
...
# 형태소 분석기가 아닌, 학습 기반 토크나이저 사용 -> 구두점 처리
questions = []
